...
public class AddEditDeliveryCompanyDialog extends JDialog {
    ...
    public AddEditDeliveryCompanyDialog(boolean edit, int id) {
        ...
        typeLabel.setText("Type of Delivery");
        ...
        typeComboBox.addItem("MEDIUM");
        typeComboBox.addItem("LARGE");

        // if the user is editing, populate the fields
        if (edit) {
            sql = "SELECT name, delivery_type, firstname, lastname, admin_id FROM delivery_company, users WHERE delivery_company.id = " + id + " AND users.id = delivery_company.admin_id";
            try (Connection connection = DriverManager.getConnection(Database.databaseUrl)) {
                Statement statement = connection.createStatement();
                ResultSet resultSet = statement.executeQuery(sql);
                if (resultSet.next()) {
                    nameTextField.setText(resultSet.getString("name"));
                    typeComboBox.setSelectedItem(resultSet.getString("delivery_type"));
    ...
    private void onOK(boolean edit, int id) {
        ...
        if (nameTextField.getText().equals("") || Objects.equals(typeComboBox.getSelectedItem(), "Select Type") || Objects.equals(adminComboBox.getSelectedItem(), "Select Admin")) {
        ...
        if (edit) {
            ...
            String currentAdminSql = "SELECT admin_id FROM delivery_company WHERE id = " + id;
            ...
            String sql = "UPDATE delivery_company SET name = ?, delivery_type = ?, admin_id = ? WHERE id = ?";
            try (Connection connection = DriverManager.getConnection(Database.databaseUrl)) {
            ...
            if (currentAdminId != Integer.parseInt(adminId)) {
                ...
                try (Connection connection = DriverManager.getConnection(Database.databaseUrl)) {
                    ...
                    preparedStatement.setString(2, "DEL_ADMIN");
                    preparedStatement.executeUpdate();
                } catch (SQLException e) {
                    e.printStackTrace();
                }
